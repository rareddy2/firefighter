...
def main():
	...
	block_dest = lab_invk(block_x, block_y, block_z, block_yaw)
	fire_dest = lab_invk(fire_x, fire_y, fire_z, fire_yaw)
	...
	move_arm(pub_cmd, loop_rate, block_dest, vel, accel)
	gripper(pub_cmd, loop_rate, True)
	time.sleep(1.0)
	if (digital_in0==True):
		move_arm(pub_cmd, loop_rate, fire_dest, vel, accel)
