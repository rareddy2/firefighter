...
targetlocationG1=[0.16,-0.17,0.0275]
targetlocationG2=[0.16,-0.12,0.0275]
targetlocationB1=[0.25,-0.17,0.0275]
targetlocationB2=[0.25,-0.12,0.0275]
targetlocationG =[0.19,0.1,0.0275]
targetlocationB =[0.25,0.1,0.0275]
...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    global targetlocationG
    global targetlocationB

    error = 0
    move_arm(pub_cmd, loop_rate, lab_invk(start_xw_yw_zw[0],
    start_xw_yw_zw[1], start_xw_yw_zw[2] + 0.1, 0), vel, accel)
    time.sleep(1)
    move_arm(pub_cmd, loop_rate, lab_invk(start_xw_yw_zw[0],
    start_xw_yw_zw[1], start_xw_yw_zw[2], 0), vel, accel)
    time.sleep(1)
    ...
    start_xw_yw_zw[1], start_xw_yw_zw[2] + 0.1, 0), vel, accel)
    time.sleep(1)
    move_arm(pub_cmd, loop_rate, lab_invk(target_xw_yw_zw[0],
    target_xw_yw_zw[1], target_xw_yw_zw[2]+ 0.1, 0), vel, accel)
    time.sleep(1)
    move_arm(pub_cmd, loop_rate, lab_invk(target_xw_yw_zw[0],
    target_xw_yw_zw[1], target_xw_yw_zw[2], 0), vel, accel)
    time.sleep(1)
...
def main():
    ...
    """
    print("B postions: " + str(xw_yw_B))
    print("G postions: " + str(xw_yw_G))
    ...
    if gc == 0:
       print("green block missing")
    if bc == 0:
      print("blue block missing")
    
    if gc != 0:
      error = move_block(pub_command, loop_rate,(G_pos[0][0],G_pos[0][1], 0.023), targetlocationG1, vel, accel)
      gc = gc - 1
    if gc != 0:
      error = move_block(pub_command, loop_rate,(G_pos[1][0],G_pos[1][1], 0.023), targetlocationG2, vel, accel)
      gc = gc - 1
    if bc != 0:
      error = move_block(pub_command, loop_rate,(B_pos[0][0],B_pos[0][1], 0.023), targetlocationB1, vel, accel)
      bc = bc - 1
    if bc != 0:
      error = move_block(pub_command, loop_rate,(B_pos[1][0],B_pos[1][1], 0.023), targetlocationB2, vel, accel)
      bc = bc - 1

    # for i in range(len(G_pos)):
    #     if i == 0:
    #         error = move_block(pub_command, loop_rate,
    # (G_pos[i][0],G_pos[i][1], 0.031), targetlocationG, vel, accel)

    #     if i == 1:
    #         if error != 1:
    #             targetlocationG = (targetlocationG[0], targetlocationG[1],targetlocationG[2]+0.035)
    #     else:
    #         targetlocationG = (targetlocationG[0], targetlocationG[1], targetlocationG[2])
    ...
    # if i == 1:
    #     if error != 1:
    #         targetlocationB = (targetlocationB[0], targetlocationB[1],
    #         targetlocationB[2]+0.031)

    #     else:
    #         targetlocationB = (targetlocationB[0], targetlocationB[1],targetlocationB[2])

    # blue_error = 1
    # error = move_block(pub_command, loop_rate,

    # (B_pos[i][0],B_pos[i][1], 0.031), targetlocationB, vel, accel)
